...
/* Code */
    ...
    //print_serial("dpb buffer: ");
    //print_serial_int(sizeof(dpbPicture_t)*(MAX_NUM_REF_IDX_L0_ACTIVE + 1));
    if (dpb->buffer == NULL) {
        ////print_serial("Couldn't allocate dpb->buffer");
        return(MEMORY_ALLOCATION_ERROR);
      }
    memset(dpb->buffer, 0,
            (MAX_NUM_REF_IDX_L0_ACTIVE + 1)*sizeof(dpbPicture_t));
    //print_serial("dppSize: ");
    //print_serial_int(dpb->dpbSize + 1);
    for (i = 0; i < dpb->dpbSize + 1; i++)
    {
        ...
        if (dpb->buffer[i].pAllocatedData == NULL) {
            //print_serial("Couldn't allocate pAllocatedData: ");
            //print_serial_int((picSizeInMbs*384 + 32+15));
            return(MEMORY_ALLOCATION_ERROR);
          } else {
            //print_serial("Allocated pAllocatedData: ");
            //print_serial_int((picSizeInMbs*384 + 32+15));
          }
    ...
    if (dpb->list == NULL || dpb->outBuf == NULL) {
        //print_serial("Couldn't allocate list / outbuf");
        return(MEMORY_ALLOCATION_ERROR);
      }
